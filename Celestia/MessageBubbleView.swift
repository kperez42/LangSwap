...
struct MessageBubbleGradient: View {
    ...
    private func replyPreviewView(replyTo: MessageReply) -> some View {
        Button {
            ...
            HStack(spacing: 8) {
                Rectangle()
                    .fill(isFromCurrentUser ? Color.white.opacity(0.5) : Color.teal.opacity(0.5))
                    .frame(width: 3)
                    .cornerRadius(2)
        ...
        HStack(spacing: 4) {
            ForEach(message.uniqueReactionEmojis, id: \.self) { emoji in
                ...
                Button {
                    ...
                    .padding(.vertical, 3)
                    .background(hasUserReacted ? Color.teal.opacity(0.2) : Color.gray.opacity(0.15))
                    .cornerRadius(12)
                    .overlay(
                        RoundedRectangle(cornerRadius: 12)
                            .stroke(hasUserReacted ? Color.teal.opacity(0.5) : Color.clear, lineWidth: 1)
                    )
                }
                .buttonStyle(.plain)
            }
